...
public class project2cs360s2019 {
	...
	public static void main(String[] args) {
		if (testing) {
			...
			for (int test_num = 0; test_num < 4; test_num++) {
				long start = System.currentTimeMillis();
				readInput("input" + test_num + ".txt");
				int result = simulateDraft();
				if (debug_solution)
					checkResult(result, test_num);
				if (debug_time)
					System.out.println(
	...
	public static int simulateDraft() {
		...
		if (!ab_pruning) {
			...
			alphabeta(root, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
		}
	...
	public static double minimax(DraftState root, int depth) {
		...
		if (root.isPlayer_turn()) {
			...
			while (!drafts.isEmpty()) {
				...
				if (Double.compare(max_val, curr_minimax_val) == 0) {
					System.out.println("Same "+curr_draft_state.last_draft);
					if (root.getNext_draft() > curr_draft_state.getLast_draft()) {
						max_val = curr_minimax_val;
						root.setMinimax_val(curr_minimax_val);
						root.setNext_draft(curr_draft_state.getLast_draft());
						System.out.println("Replacing "+root.getMinimax_val());
					}
				}
				else if (Double.compare(max_val, curr_minimax_val) < 0) {
					max_val = curr_minimax_val;
					root.setMinimax_val(curr_minimax_val);
					root.setNext_draft(curr_draft_state.getLast_draft());
				} 
		...
		// minimizing
			...
			while (!drafts.isEmpty()) {
				DraftState curr_draft_state = drafts.poll();
				double curr_minimax_val = minimax(curr_draft_state, depth + 1);
				if (Double.compare(min_val, curr_minimax_val) == 0) {
					System.out.println("Same "+curr_draft_state.last_draft);
					if (root.getNext_draft() > curr_draft_state.getLast_draft()) {
						min_val = curr_minimax_val;
						root.setMinimax_val(curr_minimax_val);
						root.setNext_draft(curr_draft_state.getLast_draft());
						System.out.println("Replacing "+root.getMinimax_val());
					}
				}
				else if (Double.compare(min_val, curr_minimax_val) > 0) {
					min_val = curr_minimax_val;
					root.setMinimax_val(curr_minimax_val);
					root.setNext_draft(curr_draft_state.getLast_draft());
				} 
	...
	public static double alphabeta(DraftState root, double alpha, double beta) {
		...
		if (root.isPlayer_turn()) {
			Queue<DraftState> drafts = new PriorityQueue<DraftState>(new ReverseAdvantageComparator());
			drafts.addAll(root.createSuccessors(heroes));
			root.setMinimax_val(Double.NEGATIVE_INFINITY);
			while (!drafts.isEmpty()) {
				DraftState curr_draft_state = drafts.poll();
				if (debug_ab)
					System.out.println(curr_draft_state);
		...
		// minimizing
			...
			Queue<DraftState> drafts = new PriorityQueue<DraftState>(new AdvantageComparator());
			drafts.addAll(root.createSuccessors(heroes));
			root.setMinimax_val(Double.POSITIVE_INFINITY);
			while (!drafts.isEmpty()) {
				DraftState curr_draft_state = drafts.poll();
				if (debug_ab)
					System.out.println(curr_draft_state);
	...
	public static void checkResult(int result, int testID) {
		...
		File file = new File("C:\\Users\\theon\\OneDrive\\Documents\\Git\\csci360\\P2_DotA_Data\\src\\test_case\\output"
				+ testID + ".txt");
